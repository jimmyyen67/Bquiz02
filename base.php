...
$alltotal  = $Total->q("select sum(`total`) from `total`");
if (empty($_SESSION['visited']) && empty($Total->find(['date' => date("Y-m-d")]))) {
    $total['date'] = date("Y-m-d");
    $total['total'] = 1;
    $Total->save($total);
    $_SESSION['visited'] = 1;
} else if (empty($_SESSION['visited']) && !empty($Total->find(['date' => date("Y-m-d")]))) {
    $total = $Total->find(['date' => date("Y-m-d")]);
    $total['total']++;
    $Total->save($total);
    $_SESSION['visited'] = 1;
